    ...
    useEffect(() => {
        ...
        const initData = async () => {
            ...
            try {
                ...
                // FIX: Preservar socios (partners) locales si Supabase los devuelve vacíos o null.
                // Mismo patrón que se usa para categories e ingredientGroups.
                const partners = (
                    (rawConfig?.partners && Array.isArray(rawConfig.partners) && rawConfig.partners.length > 0)
                        ? rawConfig.partners
                        : (globalState.config?.partners && globalState.config.partners.length > 0)
                            ? globalState.config.partners
                            : []
                );

                const configFromDB: AppConfig = rawConfig
                    ? { ...rawConfig, categories, ingredient_groups: ingredientGroups, partners }
                    : { ...globalState.config, categories, ingredient_groups: ingredientGroups, partners };
